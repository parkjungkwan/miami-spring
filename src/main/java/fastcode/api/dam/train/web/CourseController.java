...
import fastcode.api.cmm.app.web.AppController;
import fastcode.api.dam.train.service.vo.CourseVO;
import fastcode.api.util.generics.Box;
...
@RestController
@RequestMapping("/itm/courses")
public class CourseController extends AppController<CourseVO> {
	private final PageVO page;
    private final Box<CourseVO> box;
    private final ICourseService service;
    
    @PostMapping("")
    public ResponseEntity<Integer> save(@RequestBody Course param) {
        return ResponseEntity.ok(service.save(param));
    }
    @PutMapping("")
    public ResponseEntity<Integer> update(@RequestBody Course param){
    ...
    @DeleteMapping("")
    public ResponseEntity<Integer> delete(@RequestBody Course param){
    ...
    public ResponseEntity<Box<Course>> findAll(@PathVariable String pageNo) {
    	page.setPageNo(integer.apply(pageNo));
    	List<Course> list = service.findAll(page);
    	box.setList(list);
    	box.setPage(page);
