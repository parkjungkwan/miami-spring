...
import com.bitcamp.lms.itm.serviceImpl.TeamServiceImpl;

import lombok.RequiredArgsConstructor;

@RequiredArgsConstructor
@RestController
@RequestMapping("/itm/teams")
public class DamTeamWeb extends CrudController<Team>{
	private final Page page;
    private final Box<Team> box;
    private final TeamServiceImpl service;
    
    @PostMapping("")
    public ResponseEntity<Integer> save(@RequestBody Team param) {
        return ResponseEntity.ok(service.save(param));
    }
    @PutMapping("")
    public ResponseEntity<Integer> update(@RequestBody Team param){
    ...
    @DeleteMapping("")
    public ResponseEntity<Integer> delete(@RequestBody Team param){
    ...
    public ResponseEntity<Box<Team>> findAll(@PathVariable String pageNo) {
    	page.setPageNo(integer.apply(pageNo));
    	List<Team> list = service.findAll(page);
    	box.setList(list);
    	box.setPage(page);
