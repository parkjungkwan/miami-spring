package fastcode.api.sts.grade.web;
...
@RestController
@RequestMapping("/aim/grades")
public class GradeController extends CrudController<Grade>{
	private final Page page;
    private final Box<Grade> box;
    private final GradeServiceImpl service;
    
    @PostMapping("")
    public ResponseEntity<Integer> save(@RequestBody Grade param) {
        return ResponseEntity.ok(service.save(param));
    }
    @PutMapping("")
    public ResponseEntity<Integer> update(@RequestBody Grade param){
    ...
    @DeleteMapping("")
    public ResponseEntity<Integer> delete(@RequestBody Grade param){
    ...
    public ResponseEntity<Box<Grade>> findAll(@PathVariable String pageNo) {
    	page.setPageNo(integer.apply(pageNo));
    	List<Grade> list = service.findAll(page);
    	box.setList(list);
    	box.setPage(page);
        return new ResponseEntity<>(box, HttpStatus.OK);
    }
    @GetMapping("/one/{id}")
    public ResponseEntity<Grade> getOne(@PathVariable String id) {
